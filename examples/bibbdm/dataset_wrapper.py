...
:class:`torch.utils.data.Dataset` interface expected by BiBBDM training.
...
class MavicTBiBBDMDataset(Dataset):
    """A PyTorch :class:`Dataset` that loads MAVIC-T image pairs for BiBBDM.

    Each ``__getitem__`` returns ``(target_tensor, source_tensor)`` following the
    BiBBDM convention where ``a = target`` and ``b = source``.  Both tensors
