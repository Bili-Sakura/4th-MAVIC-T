...
"""Train standalone SID baseline for MAVIC-T tasks.

Usage:
    python -m examples.sid.train --task rgb2ir
    accelerate launch -m examples.sid.train --task sar2ir --train_batch_size 4
"""
...
from .trainer import SIDTrainer  # noqa: E402
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train standalone SID baseline")
    parser.add_argument("--task", choices=sorted(_TASK_TO_CONFIG.keys()), required=True)

    # Parse task first so we can expose task-specific default values as CLI args.
