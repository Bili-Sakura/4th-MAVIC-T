...
"""Sample (inference) script for a trained SID model on MAVIC-T tasks.
...
from src.models.unet.unet_2d import UNet2DWrapper as SiDUNet  # noqa: E402
...
from .dataset_wrapper import MavicTSIDDataset  # noqa: E402
...
def parse_args():
    parser = argparse.ArgumentParser(description="Sample from a trained SID model.")
...
def _load_pipeline(pretrained_path: str, primary_device: str, device_ids: list[int] | None = None) -> SIDPipeline:
    path = Path(pretrained_path)
    if not path.is_dir():
        raise ValueError(
            f"Expected a checkpoint directory, got: {pretrained_path}. "
            "SID sample currently supports diffusers-style checkpoint folders only."
        )

    logger.info("Loading SID pipeline from checkpoint directory: %s", path)
    ...
    if scheduler_config.exists():
        scheduler = SiDScheduler.from_pretrained(pretrained_path, subfolder="scheduler")
    else:
        ...
        if config_yaml.exists():
            ...
            scheduler = SiDScheduler(**scheduler_kwargs)
            logger.info(
                "Scheduler config missing under %s; reconstructed SiDScheduler from %s",
                scheduler_dir,
                config_yaml,
            )
        else:
            scheduler = SiDScheduler()
            logger.warning(
                "Scheduler config missing under %s and no config.yaml found. Using SiDScheduler defaults.",
                scheduler_dir,
            )

    pipeline = SIDPipeline(unet=unet, scheduler=scheduler).to(
...
def main():
    ...
    detailed_description = build_detailed_description(
        model_name="SID",
        model_description="SID (Simple Diffusion) - standalone simple diffusion model for image-to-image translation. PyTorch implementation. UNet backbone: diffusers UNet2DConditionModel without cross-attention.",
