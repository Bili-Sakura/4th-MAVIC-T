...
:class:`torch.utils.data.Dataset` interface expected by I2SB training.
...
class MavicTI2SBDataset(Dataset):
    """A PyTorch :class:`Dataset` that loads MAVIC-T image pairs for I2SB.

    Each ``__getitem__`` returns ``(target_tensor, source_tensor)`` following the
    I2SB convention where ``x0 = target`` and ``x_T = source``.  Both tensors
