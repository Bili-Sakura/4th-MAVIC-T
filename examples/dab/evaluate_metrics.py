...
"""Manual MAVIC-T metric evaluation for a trained DAB checkpoint on a paired validation set.

Same interface as examples/bdbm/evaluate_metrics.py.

Usage::

    conda activate rsgen
    python -m examples.dab.evaluate_metrics \
        --checkpoint_dir ./ckpt/dab/sar2eo/checkpoint-epoch-100 \
...
from examples.dab.config import TaskConfig, sar2eo_config, rgb2ir_config, sar2ir_config, sar2rgb_config
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate MAVIC-T metrics (DAB) on a paired val set.")
...
def main():
    ...
    if ema_dir.is_dir():
        ...
        pipeline.unet = DABUNet.from_pretrained(str(checkpoint_dir), subfolder="ema_unet")
