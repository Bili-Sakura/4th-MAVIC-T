#!/usr/bin/env bash
# Stage 2 — RGB→IR — Latent-space modelling, scaled-up model
# DDBM large config for latent shape (128, 128, 32)
#
# Usage:
#   bash scripts/train_stage2_rgb2ir.sh
#   # run on a specific GPU (e.g., cuda:0):
#   CUDA_VISIBLE_DEVICES=0 bash scripts/train_stage2_rgb2ir.sh
#   # (pick 0-3 to spread stages across 4 GPUs)
#   # or multi-GPU:
#   NGPU=4 bash scripts/train_stage2_rgb2ir.sh
