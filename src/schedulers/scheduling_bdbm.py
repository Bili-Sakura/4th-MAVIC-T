...
"""Brownian-bridge scheduler for the BDBM baseline.

Ported from ``libs/BDBM/model/BrownianBridge/BidirectionalBridgeModel.py``
into a diffusers-compatible scheduler API.
"""
...
class BDBMSchedulerOutput(BaseOutput):
    """Output of one BDBM sampling step."""
...
class BDBMScheduler(SchedulerMixin, ConfigMixin):
    """Bidirectional Brownian Bridge scheduler used by BDBM."""
    ...
    def get_objective(
        ...
        """Return BDBM training objective for the selected objective mode."""
    ...
    def step_b2a(
        ...
        """One reverse bridge step from ``x_t`` to ``x_{t-1}``."""
        device = x_t.device
    ...
    def step_a2b(
        ...
        """One forward bridge step from ``x_t`` to ``x_{t+1}``."""
        device = x_t.device
