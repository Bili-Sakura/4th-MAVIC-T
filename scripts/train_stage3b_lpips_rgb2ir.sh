#!/usr/bin/env bash
# Stage 3b — RGB→IR — Pixel-space LPIPS/VGG finetuning (no VAE)
...
#
# Usage:
#   bash scripts/train_stage3b_lpips_rgb2ir.sh
#   CUDA_VISIBLE_DEVICES=0 bash scripts/train_stage3b_lpips_rgb2ir.sh
#   NGPU=4 bash scripts/train_stage3b_lpips_rgb2ir.sh
...
NGPU="${NGPU:-1}"
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/train_stage3b_lpips_rgb2ir.log"
mkdir -p "${LOG_DIR}"

# --- Match Stage-3 RGB→IR pixel model architecture ---
NUM_CHANNELS=256
NUM_RES_BLOCKS=2
...
OUTPUT_DIR="${OUTPUT_DIR:-./ckpt/stage3_rgb2ir}"
