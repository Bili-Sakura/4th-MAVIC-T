...
"""Learning-rate scheduling for StegoGAN training.

StegoGAN is a feed-forward GAN — it does not use iterative noise scheduling
like diffusion models.  Instead, the "scheduler" here manages the
learning-rate decay policy used during training, identical to the CUT
scheduler pattern.

The default StegoGAN policy keeps the learning rate constant for the first
...
@dataclass
class StegoGANSchedulerOutput:
    """Output class for the StegoGAN scheduler.
...
class StegoGANScheduler:
    """Learning-rate scheduler for StegoGAN training.
    ...
    * ``"cosine"`` – Cosine annealing over total epochs.
    ...
    def step(self) -> StegoGANSchedulerOutput:
        """Advance one epoch and return the current learning rate."""
        self._scheduler.step()
        lr = self.optimizer.param_groups[0]["lr"]
        return StegoGANSchedulerOutput(lr=lr)
