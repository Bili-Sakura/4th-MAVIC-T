#!/usr/bin/env bash
# Stage 2 — CUT Ablation — SAR→EO — Pixel-space (large config)
# CUT large: ngf=128, ndf=128, n_layers_D=3 (~56.5 M, 256–512)
#
# Usage:
#   bash scripts/train_stage2_cut_sar2eo.sh
#   CUDA_VISIBLE_DEVICES=0 bash scripts/train_stage2_cut_sar2eo.sh
#   NGPU=4 bash scripts/train_stage2_cut_sar2eo.sh
...
NGPU="${NGPU:-1}"
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/train_stage2_cut_sar2eo.log"

mkdir -p "${LOG_DIR}"

# --- Large config (256–512) from configs/model_scaling_variants.yaml ---
NGF=128
NDF=128
N_LAYERS_D=3
...
OUTPUT_DIR="./ckpt/stage2_cut_sar2eo"
