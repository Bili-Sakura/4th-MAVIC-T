#!/usr/bin/env bash
# Train RGB-domain real/fake classifier with domain-specific normalization stats.
#
# Pre-computes mean/std from the RGB target domain dataset and uses them instead
# of ImageNet statistics. Run this script from the project root.
#
# Usage:
#   bash scripts/train_domain_classifier_rgb.sh
#   CUDA_VISIBLE_DEVICES=0 bash scripts/train_domain_classifier_rgb.sh
#
...
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/train_domain_classifier_rgb.log"
STATS_PATH="./stats/rgb_domain_stats.json"
...
if [ -f "${STATS_PATH}" ]; then
  ...
  echo "Training RGB-domain classifier with dataset stats: ${STATS_PATH}"
else
  echo "Training RGB-domain classifier (ImageNet normalization fallback)"
fi
