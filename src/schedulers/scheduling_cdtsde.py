...
"""Scheduler for CDTSDE (Adaptive Domain Shift Diffusion Bridge)."""
...
def space_timesteps(num_timesteps: int, section_counts: Union[str, int]) -> set[int]:
    """Create spaced timesteps (ported from CDTSDE reference)."""
...
def _extract_into_tensor(arr: torch.Tensor, timesteps: torch.Tensor, broadcast_shape) -> torch.Tensor:
    """Gather 1D tensor values at ``timesteps`` and expand to ``broadcast_shape``."""
...
class CDTSDESchedulerOutput(BaseOutput):
    """Output class for CDTSDE scheduler step."""
...
class CDTSDEScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # Training-time schedule (full length).
        ...
        train_etas = make_eta_schedule(
            schedule=eta_schedule,
            n_timestep=num_train_timesteps,
            linear_start=eta_start,
            linear_end=eta_end,
            return_t1=False,
        )

        self.train_betas = torch.tensor(train_betas, dtype=torch.float32)
        ...
        self.train_etas = torch.tensor(train_etas, dtype=torch.float32)

        # Inference-time schedule (set by set_timesteps).
        self.num_inference_steps: Optional[int] = None
        self.timesteps: Optional[torch.Tensor] = None
        self.betas: Optional[torch.Tensor] = None
    ...
    def set_timesteps(
        ...
        # Match reference behavior: sample from truncated eta horizon.
        ...
        for i, alpha_cumprod in enumerate(train_alpha_cumprod_np):
            if i in used_timesteps:
                ...
                reduced_etas.append(full_etas[i])
    ...
    def add_noise(
        ...
        """Forward diffusion (training) using full training schedule."""
    ...
    def predict_start_from_noise(
        ...
        """Recover x0 from noisy sample and predicted epsilon."""
