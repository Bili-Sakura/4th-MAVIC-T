...
"""Manual MAVIC-T metric evaluation for a trained CDTSDE checkpoint on a paired validation set.
...
Usage::
    ...
    python -m examples.cdtsde.evaluate_metrics \
        --checkpoint_dir ./ckpt/cdtsde/sar2eo/checkpoint-epoch-100 \
...
from examples.cdtsde.config import TaskConfig, sar2eo_config, rgb2ir_config, sar2ir_config, sar2rgb_config
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate MAVIC-T metrics (CDTSDE) on a paired val set.")
...
def main():
    ...
    if ema_dir.is_dir():
        ...
        pipeline.unet = CDTSDEUNet.from_pretrained(str(checkpoint_dir), subfolder="ema_unet")
    ...
    def inference_fn(source: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        result = pipeline(
            ...
            cfg_scale=cfg_scale,
            stochastic=getattr(cfg, "stochastic", True),
            apply_domain_shift=getattr(cfg, "apply_domain_shift", True),
            output_type="pt",
        )
