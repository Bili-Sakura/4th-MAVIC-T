...
"""Manual MAVIC-T metric evaluation for a trained BDBM checkpoint on a paired validation set.
...
Usage::
    ...
    python -m examples.bdbm.evaluate_metrics \
        --checkpoint_dir ./ckpt/bdbm/sar2eo/checkpoint-epoch-100 \
...
from examples.bdbm.config import TaskConfig, sar2eo_config, rgb2ir_config, sar2ir_config, sar2rgb_config
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate MAVIC-T metrics (BDBM) on a paired val set.")
...
def main():
    ...
    logger.info("Loading BDBM pipeline from %s", checkpoint_dir)
    pipeline = BDBMPipeline.from_pretrained(str(checkpoint_dir))
    ema_dir = checkpoint_dir / "ema_unet"
    if ema_dir.is_dir():
        logger.info("Loading EMA UNet from %s", ema_dir)
        pipeline.unet = BDBMUNet.from_pretrained(str(checkpoint_dir), subfolder="ema_unet")
    ...
    # In pixel space, BDBM expects symmetric channel counts; use model_channels for source.
