...
class DDBMTrainer:
    ...
    def compute_training_loss(model, scheduler, x0, x_T, pred_mode="vp",
        ...
        When *latent_target_encoder* is provided an L2 loss in VAE latent
        space is added between the denoised prediction and the target.
        """
        bsz = x0.shape[0]
        device = x0.device
        dtype = x0.dtype
        ...
        # Optional latent-space L2 loss (VAE latent alignment)
    ...
    def train(self):
        ...
        # Latent target encoder (optional VAE latent-space loss)
