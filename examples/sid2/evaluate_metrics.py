...
"""Manual MAVIC-T metric evaluation for a trained SID2 checkpoint on paired val data.
...
from examples.sid2.config import TaskConfig, rgb2ir_config, sar2eo_config, sar2ir_config, sar2rgb_config
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate MAVIC-T metrics (SID2) on paired val set.")
...
def main():
    ...
    from src.models.unet_sid import SiDUNet
    from src.pipelines.sid2 import SID2Pipeline
    from src.schedulers import SiD2Scheduler

    logger.info("Loading SID2 pipeline from %s", checkpoint_dir)
    ...
    scheduler = SiD2Scheduler.from_pretrained(str(checkpoint_dir), subfolder="scheduler")
    pipeline = SID2Pipeline(unet=unet, scheduler=scheduler).to(args.device)
    pipeline.unet.eval()
