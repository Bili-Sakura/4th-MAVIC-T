...
def rgb2ir_config(**overrides) -> TaskConfig:
    """RGB-to-IR: 3-band 1024x1024 → 1-band 1024x1024."""
    cfg = TaskConfig(
        task_name="rgb2ir",
        source_channels=3,
        target_channels=1,
        model_channels=3,  # operate in 3-ch space; 1-ch target is expanded
        resolution=256,     # train at 256; scale up later if needed
        use_augmented=True,
...
def sar2ir_config(**overrides) -> TaskConfig:
    """SAR-to-IR: 1-band 1024x1024 → 1-band 1024x1024."""
    cfg = TaskConfig(
        task_name="sar2ir",
        source_channels=1,
        target_channels=1,
...
def sar2rgb_config(**overrides) -> TaskConfig:
    """SAR-to-RGB: 1-band 1024x1024 → 3-band 1024x1024."""
    cfg = TaskConfig(
        task_name="sar2rgb",
        source_channels=1,
        target_channels=3,
