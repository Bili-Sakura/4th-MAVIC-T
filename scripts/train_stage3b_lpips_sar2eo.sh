#!/usr/bin/env bash
# Stage 3b — SAR→EO — Pixel-space LPIPS/VGG finetuning (no VAE)
...
#
# Usage:
#   bash scripts/train_stage3b_lpips_sar2eo.sh
#   CUDA_VISIBLE_DEVICES=0 bash scripts/train_stage3b_lpips_sar2eo.sh
#   NGPU=4 bash scripts/train_stage3b_lpips_sar2eo.sh
...
NGPU="${NGPU:-1}"
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/train_stage3b_lpips_sar2eo.log"
mkdir -p "${LOG_DIR}"

# --- Match Stage-3 SAR→EO pixel model architecture ---
NUM_CHANNELS=128
NUM_RES_BLOCKS=2
...
OUTPUT_DIR="${OUTPUT_DIR:-./ckpt/stage3_sar2eo}"
