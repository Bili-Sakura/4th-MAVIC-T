...
"""Pipeline for conditional Simpler Diffusion (SiD2) image translation."""
...
from src.schedulers.scheduling_sid2 import SiD2Scheduler


@dataclass
class SID2PipelineOutput(BaseOutput):
    """Output class for SID2 pipeline."""

    images: Union[List[Image.Image], np.ndarray, torch.Tensor]
    nfe: int = 0


class SID2Pipeline(DiffusionPipeline):
    """Conditional Simpler Diffusion pipeline for paired translation."""
    ...
    def __call__(
        ...
        """Translate ``source_image`` with conditional SiD2 denoising."""
        device = self.device
        dtype = self.dtype
        ...
        nfe_count = 0
        progress_bar = tqdm(range(num_inference_steps), desc="SID2 Sampling")
        ...
        return SID2PipelineOutput(images=images, nfe=nfe_count)
