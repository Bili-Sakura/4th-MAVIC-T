...
"""Manual MAVIC-T metric evaluation for a trained BiBBDM checkpoint on a paired validation set.
...
Usage::
    ...
    python -m examples.bibbdm.evaluate_metrics \
        --checkpoint_dir ./ckpt/bibbdm/sar2eo/checkpoint-epoch-100 \
...
from examples.bibbdm.config import TaskConfig, sar2eo_config, rgb2ir_config, sar2ir_config, sar2rgb_config
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate MAVIC-T metrics (BiBBDM) on a paired val set.")
...
def main():
    ...
    if ema_dir.is_dir():
        ...
        pipeline.unet = BiBBDMUNet.from_pretrained(str(checkpoint_dir), subfolder="ema_unet")
    ...
    # In pixel space, BiBBDM expects symmetric channel counts; use model_channels for source.
