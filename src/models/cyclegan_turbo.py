"""CycleGAN-Turbo model built on ``diffusers`` components.
...
* **Dual VAEs** (``vae_a2b`` and ``vae_b2a``) with skip-connection-enhanced
  decoders for bidirectional image translation.
* **Shared UNet** with LoRA adapters split into encoder/decoder/other groups.
* **Single-step denoising** at ``t = 999``.
"""
...
from src.schedulers.scheduling_turbo import make_1step_sched
from src.utils.vae_utils import vae_encoder_fwd, vae_decoder_fwd


# ---------------------------------------------------------------------------
# VAE encode / decode helpers for bidirectional translation
# ---------------------------------------------------------------------------

class VAE_encode(nn.Module):
    """Encode an image to latent space, selecting the appropriate VAE by direction."""
...
class VAE_decode(nn.Module):
    """Decode latents to image space with skip connections, selecting VAE by direction."""
...
# ---------------------------------------------------------------------------
# UNet / VAE initialisation helpers
# ---------------------------------------------------------------------------

def initialize_unet(rank, return_lora_module_names=False):
    """Create a UNet with separate LoRA adapters for encoder / decoder / other blocks.

    Returns the UNet (and optionally the three lists of module names).
    """
...
def initialize_vae(rank=4, return_lora_module_names=False):
    """Create a VAE with skip connections and LoRA adapters.

    Returns the VAE (and optionally the list of LoRA target module names).
    """
...
# ---------------------------------------------------------------------------
# CycleGAN-Turbo model
# ---------------------------------------------------------------------------

class CycleGANTurbo(torch.nn.Module):
    """CycleGAN-Turbo for unpaired bidirectional image-to-image translation.

    Parameters
    ----------
    pretrained_path : str or None
        Path to a saved checkpoint (``*.pkl``).
    ...
    def __init__(
        ...
        if pretrained_path is not None:
            ...
            # No pretrained – caller must set up adapters
            self.timesteps = torch.tensor([999], device="cuda").long()
            self.caption = None
            self.direction = None
    ...
    def _load_ckpt_from_state_dict(self, sd):
        """Load LoRA weights from a checkpoint state dict."""
    ...
    def forward_with_networks(x, direction, vae_enc, unet, vae_dec, sched, timesteps, text_emb):
        """Run the encode → denoise → decode pipeline with explicit network references.

        This static method is used during training where network references come from
        accelerator-wrapped modules.
        """
        B = x.shape[0]
        assert direction in ["a2b", "b2a"]
    ...
    @staticmethod
    def get_trainable_params(unet, vae_a2b, vae_b2a):
        """Collect all trainable parameters from the UNet and both VAEs."""
    ...
    def forward(self, x_t, direction=None, caption=None, caption_emb=None):
        ...
        direction : str or None
            ``"a2b"`` or ``"b2a"``. Uses ``self.direction`` if ``None``.
        caption : str or None
            Text prompt. Uses ``self.caption`` if both *caption* and
            *caption_emb* are ``None``.
