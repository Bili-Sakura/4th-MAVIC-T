#!/usr/bin/env bash
# Train EO-domain real/fake classifier with domain-specific normalization stats.
#
# Pre-computes mean/std from the EO target domain dataset and uses them instead
# of ImageNet statistics. Run this script from the project root.
#
# Usage:
#   bash scripts/train_domain_classifier_eo.sh
#   CUDA_VISIBLE_DEVICES=0 bash scripts/train_domain_classifier_eo.sh
#
...
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/train_domain_classifier_eo.log"
STATS_PATH="./stats/eo_domain_stats.json"
...
if [ -f "${STATS_PATH}" ]; then
  ...
  echo "Training EO-domain classifier with dataset stats: ${STATS_PATH}"
else
  echo "Training EO-domain classifier (ImageNet normalization fallback)"
fi
