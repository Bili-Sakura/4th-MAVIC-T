...
class EDMBiBBDMUNet(ModelMixin, ConfigMixin):
    """EDM-style BiBBDM UNet with Fourier time embedding."""
...
class EDM2BiBBDMUNet(ModelMixin, ConfigMixin):
    ...
    def forward(self, x_t, timesteps, context=None):
        sigma = timesteps.float().reshape(-1, 1, 1, 1)
        ...
        x_precond = c_in * x_t
        if self.condition_mode == "concat" and context is not None:
            x_precond = torch.cat([x_precond, context], dim=1)
        F_x = self.unet(x_precond, c_noise).sample
        return c_skip * x_t + c_out * F_x


class VDMBiBBDMUNet(ModelMixin, ConfigMixin):
    """VDM-style BiBBDM UNet with logSNR time normalization."""
    ...
    def forward(self, x_t, timesteps, context=None):
        t_normalized = (timesteps.float() - self.gamma_min) / (self.gamma_max - self.gamma_min)
        if self.condition_mode == "concat" and context is not None:
            x_t = torch.cat([x_t, context], dim=1)
