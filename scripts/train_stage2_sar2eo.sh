#!/usr/bin/env bash
# Stage 2 — SAR→EO — Latent-space modelling, scaled-up model
# DDBM medium config for latent shape (32, 32, 32)
#
# Usage:
#   bash scripts/train_stage2_sar2eo.sh
#   # run on a specific GPU (e.g., cuda:0):
#   CUDA_VISIBLE_DEVICES=0 bash scripts/train_stage2_sar2eo.sh
#   # (pick 0-3 to spread stages across 4 GPUs)
#   # or multi-GPU:
#   NGPU=4 bash scripts/train_stage2_sar2eo.sh
...
NGPU="${NGPU:-1}"
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/train_stage2_sar2eo.log"
