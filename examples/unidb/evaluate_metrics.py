#!/usr/bin/env python
"""Manual MAVIC-T metric evaluation for a trained UniDB checkpoint on a paired validation set.
...
Usage::
    ...
    python -m examples.unidb.evaluate_metrics \
        --checkpoint_dir ./ckpt/unidb/sar2eo/checkpoint-epoch-100 \
...
from examples.unidb.config import TaskConfig, sar2eo_config, rgb2ir_config, sar2ir_config, sar2rgb_config
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate MAVIC-T metrics (UniDB) on a paired val set.")
...
def main():
    ...
    if ema_dir.is_dir():
        ...
        pipeline.unet = UniDBConditionalUNet.from_pretrained(str(checkpoint_dir), subfolder="ema_unet")
