...
Usage:
    # DDBM (e.g. checkpoint-10000 from stage1_sar2eo)
    python -m examples.pipelines.run_inference \
        --model ddbm \
        --checkpoint ./ckpt/exp3/stage1_sar2eo/ddbm/sar2eo/checkpoint-10000 \
        --input_dir ./path/to/images \
        --output_dir ./outputs \
        --num_steps 1000

    # With custom_pipeline via DiffusionPipeline.from_pretrained
    python -c "
from diffusers import DiffusionPipeline
from examples.pipelines.ddbm.pipeline import DDBMPipeline, DDBMUNet, DDBMScheduler

ckpt = './ckpt/exp3/stage1_sar2eo/ddbm/sar2eo/checkpoint-10000'
unet = DDBMUNet.from_pretrained(ckpt, subfolder='ema_unet')
scheduler = DDBMScheduler.from_pretrained(ckpt, subfolder='scheduler')
pipe = DDBMPipeline(unet=unet, scheduler=scheduler)
pipe = pipe.to('cuda')
# ... run inference
"
"""
...
def load_ddbm_pipeline(checkpoint: str, device: str = "cuda", use_ema: bool = True):
    """Load DDBM pipeline from checkpoint - no external imports."""
    from examples.pipelines.ddbm.pipeline import DDBMPipeline, DDBMUNet, DDBMScheduler

    ckpt = Path(checkpoint)
    # Prefer ema_unet; DDBMUNet.from_pretrained handles ema_unet without config.json
...
def main():
    ...
    parser.add_argument("--model", choices=["ddbm", "dbim", "ddib", "i2sb", "bibbdm", "bdbm", "cdtsde"], required=True)
    ...
    for i, img_path in enumerate(inputs):
        ...
        if img.mode == "L":
            img = img.convert("RGB")  # grayscale: keep as L for 1-channel tasks
        import numpy as np
