#!/usr/bin/env bash
# Stage 1 — CUT Ablation — SAR→RGB — Pixel-space (large config, 512 crop)
# CUT large: ngf=128, ndf=128, n_layers_D=3 (~56.5 M, 256–512)
#
# Usage:
#   bash scripts/train_stage1_cut_sar2rgb.sh
#   CUDA_VISIBLE_DEVICES=3 bash scripts/train_stage1_cut_sar2rgb.sh
#   NGPU=4 bash scripts/train_stage1_cut_sar2rgb.sh
...
NGPU="${NGPU:-1}"
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/train_stage1_cut_sar2rgb.log"
...
OUTPUT_DIR="./ckpt/stage1_cut_sar2rgb"
