#!/usr/bin/env bash
# Stage 2 — CUT Ablation — RGB→IR — Pixel-space (huge config, 512 crop)
# CUT huge: ngf=256, ndf=256, n_layers_D=4 (~293 M, 512–1024)
#
# Usage:
#   bash scripts/train_stage2_cut_rgb2ir.sh
#   CUDA_VISIBLE_DEVICES=0 bash scripts/train_stage2_cut_rgb2ir.sh
#   NGPU=4 bash scripts/train_stage2_cut_rgb2ir.sh
...
NGPU="${NGPU:-1}"
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/train_stage2_cut_rgb2ir.log"
...
OUTPUT_DIR="./ckpt/stage2_cut_rgb2ir"
